...
void newEntry() // function to input new student data
{
	...
	outFile.open("StudentGPA.dat",ios::app); // opens StudentGPA file and appends to the end
	if (outFile.is_open())
	{
...
void prntRpt() // function to print out student records
{
	...
	inFile.open("StudentGPA.dat",ios::in); // opens and reads from StudentGPA file
	if (inFile.is_open())
	{
		inFile >> studID;
...
void getGPA (float &gpa) // function to input student GPA
{
	do
	{
		...
		if (gpa < 0.0 || gpa > 4.0) // prints an error if the GPA is not within range
		{
			cout << endl << "Invalid GPA." << endl;
		}
	} while (gpa < 0.0 || gpa > 4.0); // repeats until a correct GPA is entered
} // end getGPA
...
void prntStanding (float studGPA, string first, string last) // function to print the academic standing of the student
{
	...
	else if (studGPA >= 2.0 && studGPA <= 3.65)
	{
